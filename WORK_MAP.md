# خريطة عمل منصة وصل - Wasl Command Center

> ⚠️ **تنبيه الأقدمية:** هذه الوثيقة تاريخية (آخر تحديث شامل 2026-09-06).
> **المصدر المرجعي الوحيد المعتمد هو `REFERENCE.md`** (محدث 2026-09-10) — أي تعارض يُحسم لصالحه.
...
## 12. الباقي (خارطة الطريق — محدثة 2026-09-10، تحل محل القائمة القديمة)

- [x] Dashboard مرئية + بوابة عميل مستقلة (`client.html` على `/portal/`)
- [x] Kill switch + خطط trial/basic/clinic/pro + انتهاء تجربة مُنفذ
- [x] نقل Takeover/حالة الحجز/CSAT إلى DB (تم — `store.mjs` + `kv_store`)
- [x] Triage العيادة + قائمة الانتظار + تعبئة تلقائية
- [x] إشعار موظف فوري (تصعيد/حجز/دفع/مراجعة) + تنبيهات المالك واتساب
- [x] الدينار افتراضياً + محافظ/CliQ حصراً (Stripe محذوف نهائياً — البنود القديمة أدناه ملغاة)
- [x] منع الحجز المزدوج (قيد فريد + موعد واحد/وقت) + إلغاء يحرر الموعد
...
*آخر تحديث: 2026-09-10 - يشمل كل الشغل حتى منظور العميل (`660f9bb`) — التفاصيل الكاملة في `REFERENCE.md`*
