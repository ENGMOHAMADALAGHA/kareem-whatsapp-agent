...
// ──────────────────────────────────────────────
const SYSTEM_PROMPT = `
أنت "كريم"، ممثل مبيعات آلي لبق، مباشر، ومهني لمتجر مستلزمات رياضية على واتساب.

# هويتك:
- اسمك كريم، تتحدث العربية بلهجة ودودة ومهنية.
- أسلوبك مختصر، واضح، ومهذب. لا تكن ثرثاراً.
- ترد دائماً باللغة العربية (نفس لغة العميل).

# الشفافية:
- إذا سُئلت "هل أنت ذكاء اصطناعي؟ / بوت؟ / إنسان؟" أجب بثقة وشفافية: "نعم، أنا كريم، مساعد ذكي آلي لمتجر المستلزمات الرياضية" ثم أكمل المساعدة في البيع بسلاسة.
...
# قواعد البيع الصارمة:
- ممنوع تماماً اقتراح منتجات أو أسعار غير مذكورة أعلاه.
- إذا اعترض العميل على السعر: وضّح القيمة (جودة، متانة، راحة، ضمان) أو اقترح المنتج المكمل الآخر كبديل أو إضافة.
- كن مباشراً وشجّع على الشراء بلطف دون إلحاح مزعج.
...
function mockReply(userMessage) {
  ...
  if (/(هل انت|هل أنت|بوت|روبوت|ذكاء اصطناعي)/.test(msg)) {
    return {
      reply: "نعم، أنا كريم مساعد ذكي آلي لمتجر المستلزمات الرياضية، وسعيد بمساعدتك! لدينا حذاء ركض احترافي بـ $50 وحزام دعم الظهر بـ $20 (+$5 توصيل). كيف أساعدك اليوم؟",
      transfer_to_human: false,
      intent: "استفسار",
    };
  }
  ...
  if (/(اريد.*موظف|أريد.*موظف|اريد.*انسان|أريد.*إنسان|اريد.*بشري|أريد.*بشري|اتحدث.*موظف|أتحدث.*موظف|اكلم.*موظف|أكلم.*موظف|التحدث مع.*موظف|التحدث مع.*انسان|التحدث مع.*إنسان|كلم.*موظف|حولني.*موظف|حولني.*انسان|حولني.*إنسان|خدمة عملاء|بشري لو سمحت|موظف بشري)/.test(msg)) {
    return {
      reply: "بالتأكيد، قمت بإبلاغ الفريق البشري وسيتواصل معك أحد الموظفين في أقرب وقت. شكراً لصبرك!",
  ...
  if (/(غالي|كثير|سعر مرتفع|ما تقدر تنقص|تخفيض|خصم|ليه كذا سعر)/.test(msg)) {
    return {
      reply: "أتفهم وجهة نظرك تماماً. الحذاء مصنوع بخامات احترافية عالية الجودة لراحة ودعم القدم لمسافات طويلة ويستحق الاستثمار. كخيار اقتصادي ممتاز، حزام دعم الظهر متوفر بـ $20 فقط (+$5 توصيل بالإجمالي $25). هل ترغب به بدلاً منه أو كإضافة؟",
      transfer_to_human: false,
  ...
  if (/(اشتري|أشتري|اطلب|أطلب|اريد.*حذاء|أريد.*حذاء|اريد.*حزام|أريد.*حزام|احجز|أحجز|تم.*الشراء|موافق.*اطلب)/.test(msg)) {
    ...
    if (wantsShoes && wantsBelt) {
      return {
        reply: "ممتاز! حذاء الركض $50 + حزام دعم الظهر $20 + التوصيل $5 = الإجمالي $75. هل تؤكد الطلب لإرساله لك؟",
        transfer_to_human: false,
        intent: "شراء",
      };
    }
    if (wantsBelt) {
      return {
        reply: "رائع! حزام دعم الظهر سعره $20 + $5 توصيل = الإجمالي $25. هل تؤكد الطلب؟",
        transfer_to_human: false,
        intent: "شراء",
      };
    }
    return {
      reply: "ممتاز! حذاء الركض الاحترافي سعره $50 + $5 توصيل = الإجمالي $55. هل تؤكد الطلب لإرساله لك؟",
      transfer_to_human: false,
      intent: "شراء",
    };
  }
  ...
  if (/(كم.*سعر|بكم|سعر.*حذاء|سعر.*حزام|توصيل|منتجات|عندكم ايش|وش عندكم|ماذا لديكم)/.test(msg)) {
    return {
      reply: "لدينا: حذاء ركض احترافي بـ $50، وحزام دعم الظهر بـ $20، ورسوم التوصيل ثابتة $5. أي منتج يهمك أكثر؟",
      transfer_to_human: false,
      intent: "استفسار",
    };
  }
  // ترحيب أو رسالة عامة
  return {
    reply: "أهلاً بك! أنا كريم من متجر المستلزمات الرياضية. لدينا حذاء ركض احترافي ($50) وحزام دعم الظهر ($20) مع توصيل ثابت $5. كيف أساعدك اليوم؟",
    transfer_to_human: false,
    intent: "استفسار",
  };
}
